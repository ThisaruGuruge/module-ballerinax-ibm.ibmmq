...
import static io.ballerina.lib.ibm.ibmmq.listener.Listener.NATIVE_CONSUMER;
...
public class BallerinaIbmmqListener implements MessageListener {
    ...
    BallerinaIbmmqListener(Environment environment, BObject service, IbmmqService nativeService) {
    ...
    public void onMessage(Message message) {
        Thread.startVirtualThread(() -> {
            try {
                ...
                this.environment.getRuntime().callMethod(this.service, ON_MESSAGE, strandMetadata, ballerinaMessage);
                MessageConsumer consumer = (MessageConsumer) this.service.getNativeData(NATIVE_CONSUMER);
            } catch (Throwable e) {
                boolean isIsolated = this.nativeService.isOnErrorIsolated();
                this.environment.getRuntime().callMethod(this.service, ON_ERROR,
                        new StrandMetadata(isIsolated, null),
                        createError(IBMMQ_ERROR, "Failed to fetch the message", e));
            }
        });
    }
}
